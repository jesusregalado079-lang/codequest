...
**Save reliability (2026-09-23) — answers must never fail to save.** A parent reported the check-work page showing "not answered" for work the boy had done. Reproduced in a real browser against the deployed build, three separate ways: (1) *the check page opened on today, not on the day that needed checking* — on a Wednesday with only Monday submitted, every item read "Not answered" while the answers were in fact saved (the most likely cause of the report); (2) a stale second tab wrote its old copy of the whole store over newer answers; (3) when localStorage was full, `setItem` threw, nothing was saved, and nothing told anyone. Fixes, all in `src/daily/`: `store.js` `commit(current, change, opts)` is the one write path — it applies each change to the LATEST saved answers (not the tab's stale in-memory copy), never throws, and returns `{ core, ink }`; the in-memory copy is used as the base only if the saved one is missing/unreadable or the last save failed, so nothing typed is lost; `refreshed()` lets a tab adopt newer saved answers (used on render, on `storage` events, and when the tab becomes visible). Drawings moved to their own key `codequest-daily-ink-v1` with a fixed budget (`INK_BUDGET`, oldest sheets evicted first, never the sheet being drawn) and are the first thing dropped when the ~5MB origin quota is full, so scratch work can never block answers; a store saved by the earlier version (drawings inside the answers blob) migrates on `load()`, drawings first. `drawing.js` `encodeInk()` caps one saved frame (Safari cannot encode WebP and returns a much bigger PNG). If even that fails, `app.js` shows a neutral fixed banner ("This iPad is not saving right now...") and keeps the answers in memory, retrying on every later change. `navigator.storage.persist()` is requested best-effort. `parent-view.js` `suggestedDay()` opens check-work on the latest day waiting for a parent, else today if it has work, else the latest day with any work; an empty day names the days that do have work. The PC hub (`src/progress.js`) already re-reads before every write and its lesson UI already reports failed saves, so it needed no change. Guarded by `test/daily-page.test.js` (quota-limited storage, two-tab, wiped storage, migration, ink budget, `suggestedDay`) and the browser suite's `persistence` and `migration` sections (`.foreman/scratch/browser/daily-regression.cjs`).
...
## 2a. Week 11 standard track, Tuesday to Friday — re-authored 2026-09-23
The 10-year-old finished Monday (the old fixed 100-square grid on every problem) and stalled: the jump from "percent of 100" to "6 out of 8" was too big, and he was still not breaking a problem into smaller pieces. Monday stays as it was. Tuesday to Friday were rewritten as one ramp built on a **block bar that fits each question** (`src/daily/blocks.js`): 6 out of 8 draws 8 blocks (12.5% each); a big whole is grouped into friendly blocks (3 out of 12 is 4 blocks of 3, 25% each; wholes up to 10 get one block per item, otherwise the block count is 4, 5, 10 or 20 where the numbers divide cleanly). The same three steps every time: count the blocks in the whole, find what ONE block is worth (100 divided by the block count), count the blocks you have and multiply. How much the bar labels for him fades by weekday, authored per item as `help`: Tuesday `all` (every block labeled, running total read out — deliberately close to a worked example), Wednesday `unit` (only what one block is worth), Thursday `clues` (one or two randomly chosen blocks labeled, seeded by the item id so they are the same on every reload, never the block the answer lands on), Friday `none` (just "the whole is N blocks = 100%"). He still TYPES the percent; the blocks are working-out only, saved separately (`store.blockFills`) and never graded. Neutrality holds: `blockView` never reads `item.answer` or what he typed, so nothing on his screen says right or wrong (tested). Each Tue-Fri math sheet opens with a plain-language intro and a worked example (Friday sheets intentionally have none). "X% of a number" problems are dropped this week so the skill is learned as "part out of whole" first; they can return once it clicks. The guided track (younger son) is unchanged.
